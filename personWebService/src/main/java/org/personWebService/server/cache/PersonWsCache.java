 ...
 * @param <V> 
 */
public class PersonWsCache<K,V> {
   ...
   * @param theCache
   */
  public PersonWsCache(Cache theCache) {
    this.cache = theCache;
  }
   ...
   */
  public PersonWsCache(String cacheName) {
    this(PwsEhcacheController.ehcacheController().getCache(cacheName));
  }
   ...
   */
  public PersonWsCache(String cacheName, int defaultMaxElementsInMemory, 
      boolean defaultEternal, int defaultTimeToIdleSeconds, 
      int defaultTimeToLiveSeconds, boolean defaultOverflowToDisk) {
    this(PwsEhcacheController.ehcacheController().getCache(cacheName, true, defaultMaxElementsInMemory, defaultEternal, defaultTimeToIdleSeconds, defaultTimeToLiveSeconds, defaultOverflowToDisk));
    this.clear();
  }
  ...
  public synchronized Set<K> keySet() {
    List keyList = PersonWsServerUtils.nonNull(this.cache.getKeys());
