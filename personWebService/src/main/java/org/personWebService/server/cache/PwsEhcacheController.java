 ...
 * @version $Id: PwsEhcacheController.java,v 1.1 2013/06/20 06:02:51 mchyzer Exp $
 */
public class PwsEhcacheController implements PwsCacheController {

  /**
   * singleton cache controller
   */
  private static PwsEhcacheController ehcacheController = null;

  /**
   * utility cache controller if you dont want to create your own...
   * @return ehcache controller
   */
  public static PwsEhcacheController ehcacheController() {
    if (ehcacheController == null) {
      ehcacheController = new PwsEhcacheController();
    }
   ...
   */
  public PersonWsCache getTwoFactorCache(String name) 
  ...
  /**
   * Retrieve a PersonWsCache which is a generic Map cache.  Note the defaults are only used
   ...
   * @throws  IllegalStateException if cache not found.
   */
  public PersonWsCache getTwoFactorCache(String name, boolean useDefaultIfNotInConfigFile,
    ...
    throws  IllegalStateException { 
    return new PersonWsCache(this.getCache(name, useDefaultIfNotInConfigFile, 
        defaultMaxElementsInMemory, defaultEternal, defaultTimeToIdleSeconds, 
        defaultTimeToLiveSeconds, defaultOverflowToDisk));
  }
  
  /**
   * Note, this might be better to be used from PersonWsCache
   * @param name should be unique, prefix with fully qualified classname
   * @return  Cache <i>name</i>.
   * @throws  IllegalStateException if cache not found.
   * @since   1.2.1
  ...
  /** logger */
  private static final Log LOG = PersonWsServerUtils.getLog(PwsEhcacheController.class);
   ...
   * @since   1.2.1
   */
  public PwsCacheStats getStats(String cache) {
    ...
    return new PwsEhcacheStats( c.getStatistics() );
  }
  ...
  public void initialize() {
    if (this.mgr == null) {
      synchronized(PwsEhcacheController.class) {
        if (this.mgr == null) {
