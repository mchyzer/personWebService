 ...
 * 
 */
public class PersonWsFilterJ2ee implements Filter {

  /** logger */
  private static final Log LOG = LogFactory.getLog(PersonWsFilterJ2ee.class);
  ...
  public static long retrieveRequestStartMillis() {
    ...
    return PersonWsServerUtils.longValue(requestStartMillis, 0);
  }
  ...
  public static boolean allowedToActAsOtherUsers() {
    ...
    if (!StringUtils.isBlank(originalLoginid)) {
      ...
      PersonWsAuthorizationInterface personWsAuthorizationInterface = PersonWebServiceServerConfig.retrieveConfig().personWsAuthorization();
      
      Set<String> adminsAllowedToActAsOtherUsersSet = personWsAuthorizationInterface.adminUserIdsWhoCanBackdoorAsOtherUsers();
      
      //if there are admins configured (e.g. non prod)
      if (PersonWsServerUtils.length(adminsAllowedToActAsOtherUsersSet) > 0) {
        
        //see if loginid is there, might change something...
        originalLoginid = PersonWsFilterJ2ee.retrieveUserIdFromRequestOriginalNotActAs(false);
  ...
  public static String retrieveUserIdFromRequestOriginalNotActAs(boolean fromUi) {
    ...
    if (principal == null) {
      ...
      if (PersonWsServerUtils.isBlank(principalName)) {
  ...
  public static String retrieveUserIdFromRequest(boolean fromUi) {
    ...
    if (allowedToActAsOtherUsers()) {
      ...
      String urlActAsLoginid = httpServletRequest.getParameter("pwsBackdoorLoginName");

      if (httpServletRequest.getParameterMap().keySet().contains("pwsBackdoorLoginName")) {
        ...
        if (!StringUtils.isBlank(urlActAsLoginid)) {
          ...
          httpSession.setAttribute("personWsActAsUserId", urlActAsLoginid);
          
        } else {
          
          httpSession.removeAttribute("personWsActAsUserId");
        }
      ...
      if (httpSession != null) {
        ...
        String actAsLogind = (String)httpSession.getAttribute("personWsActAsUserId");
    ...
    PersonWsServerUtils.assertion(PersonWsServerUtils.isNotBlank(principalName),
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
    ...
    try {
      ...
      if (PersonWebServiceServerConfig.retrieveConfig().propertyValueBoolean("personWsServer.autoProxyWrapper", false)) {
      ...
      request = new PwsHttpServletRequest((HttpServletRequest)request);
  ...
  public static List<Cookie> findCookiesByPrefix(String name) {
    ...
    int cookiesLength = PersonWsServerUtils.length(cookies);
