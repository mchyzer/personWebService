...
import org.personWebService.server.status.DiagnosticTask;
import org.personWebService.server.status.DiagnosticType;
import org.personWebService.server.util.PersonWsServerUtils;
import org.personWebService.server.util.PwsServerVersion;



/**
 * status servlet to see if open two factor is ok (e.g. the DB and the loader jobs, etc)
 * @author mchyzer
 *
 */
@SuppressWarnings("serial")
public class PersonWsStatusServlet extends HttpServlet {
  ...
  static {
    PersonWsStatusServlet.registerStartup();
  }

  /** logger */
  private static final Log LOG = PersonWsServerUtils.getLog(PersonWsStatusServlet.class);
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      result.append(PersonWsServerUtils.hostname());
      
      String version = null;
      try {
        version =PwsServerVersion.currentVersionString();
      } catch (Exception e) {
        version = "Version not found";
        
      }
      
      result.append(", openTwoFactorVersion: ").append(version);
      ...
      try {
        ...
        PersonWsServerUtils.closeQuietly(writer);
      }
