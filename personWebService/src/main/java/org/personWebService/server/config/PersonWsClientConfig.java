...
public class PersonWsClientConfig {
   ...
   */
  private static PersonWsCache<String, PersonWsClientConfig> clientConfigCache = new PersonWsCache<String, PersonWsClientConfig>(
      "org.personWebService.server.config.PersonWsClientConfig.clientConfigCache", 1000, false, 60, 60, false);
  
  /**
   * cache to hold which client configs dont exist
   */
  private static PersonWsCache<String, Boolean> clientConfigCacheNotExist = new PersonWsCache<String, Boolean>(
      "org.personWebService.server.config.PersonWsClientConfig.clientConfigCache", 1000, false, 60, 60, false);
    ...
    if (personWsClientConfig[0] == null) {
      ...
      if (personWsClientConfig[0] == null && !hasBuiltConfigsOnce) {
        ...
        for (int i=0;i<10;i++) {
          ...
          PersonWsServerUtils.sleep(1000);
