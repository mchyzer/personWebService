...
import org.personWebService.server.j2ee.PersonWsStatusServlet;
...
public abstract class DiagnosticTask {
  ...
  protected boolean ignoringDiagnostic() {
    ...
    boolean ignoreDiagnostics = PersonWebServiceServerConfig.retrieveConfig().propertyValueBoolean("status.diagnostic.ignore." + this.retrieveName(), false);
  ...
  public void appendSuccessTextLine(String text){
    ...
    this.retrieveSuccessText().append("SUCCESS ").append(this.retrieveName())
      .append(": ").append(text).append(PersonWsStatusServlet.elapsedSuffix() + "\n");
  }
