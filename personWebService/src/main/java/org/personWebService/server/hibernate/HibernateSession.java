...
public class HibernateSession {
  ...
  /** logger */
  private static final Log LOG = PersonWsServerUtils.getLog(HibernateSession.class);
  ...
  private HibernateSession(HibernateSession parentHibernateSession,
      PersonWsTransactionType transactionType) throws PwsDaoException {

    //if readonly, then dont allow read/write transactions
    if (isReadonlyMode()) {
      if (transactionType != null && transactionType.isTransactional()) {
        transactionType = PersonWsTransactionType.READONLY_OR_USE_EXISTING;
      }
    }
    
    //if we arent using nested transactions, then just use parent if there is one...
    if (!PersonWebServiceServerConfig.retrieveConfig().propertyValueBoolean("ddlutils.use.nestedTransactions", true) && parentHibernateSession != null) {
      ...
      if (transactionType.isNewAutonomous()) {
        if (transactionType == PersonWsTransactionType.READ_WRITE_NEW) {
          transactionType = PersonWsTransactionType.READ_WRITE_OR_USE_EXISTING;
        } else if (transactionType == PersonWsTransactionType.READONLY_NEW) {
          transactionType = PersonWsTransactionType.READONLY_OR_USE_EXISTING;
        }
      }
    ...
    if (this.isNewHibernateSession()) {
      ...
      if (!PersonWsTransactionType.NONE.equals(transactionType)) {
        this.immediateSession = PersonWsDaoFactory.getFactory().getSession();
      }
      ...
      if (!this.immediateTransactionTypeUsed.isReadonly()) {
        ...
        String useSavepointsString = PersonWebServiceServerConfig.retrieveConfig().propertyValueString("jdbc.useSavePoints");
        boolean useSavepoints;
        if (StringUtils.isBlank(useSavepointsString)) {
          useSavepoints = !PersonWsServerUtils.isHsql();
        } else {
          useSavepoints = PersonWsServerUtils.booleanValue(useSavepointsString);
        }
        ...
        if (useSavepoints && (parentHibernateSession != null   // && this.activeHibernateSession().isTransactionActive()  && !this.activeHibernateSession().isReadonly() 
            || PersonWebServiceServerConfig.retrieveConfig().propertyValueBoolean("jdbc.useSavePointsOnAllNewTransactions", false))) {
          try {
        ...
        } else if (PersonWsServerUtils.isHsql() && parentHibernateSession != null) {
          //do this for tests...
          savePointCount++;
        }
      }
   ...
   */
  private PersonWsTransactionType immediateTransactionTypeUsed = null;

  /**
   * the transaction type this was setup as. this is the one declared in
   * callback
   */
  private PersonWsTransactionType immediateTransactionTypeDeclared = null;
  ...
  private static HibernateSession staticHibernateSession() {
    ...
    return (HibernateSession) PersonWsServerUtils.get(hibSet, size - 1);
  }
   ...
   * @throws PwsDaoException
   */
  public static HibernateSession _internal_hibernateSession(PersonWsTransactionType transactionType) throws PwsDaoException {
  ...
  public static void assertNotReadonly() {
    if (PersonWebServiceServerConfig.retrieveConfig().propertyValueBoolean("twoFactor.api.readonly", false)) {
   ...
   */
  @SuppressWarnings("deprecation")
  public static void _internal_hibernateSessionCatch(HibernateSession hibernateSession, Throwable e) throws PwsDaoException {
    ...
    PersonWsServerUtils.logErrorNextException(LOG, e, 100);
    
    String errorString = "Problem in HibernateSession: " + hibernateSession;
    // rethrow
    if (e instanceof PwsDaoException) {
      if (!PersonWsServerUtils.injectInException(e, errorString)) {
        LOG.error(errorString);
      }
      throw (PwsDaoException) e;
    }
    if (e instanceof StaleObjectStateException) {
      throw new PwsStaleObjectStateException(errorString, e);
    }
    // if hibernate exception, repackage
    if (e instanceof HibernateException) {
      throw new PwsDaoException(errorString, e);
    }
    ...
    // if exception and not handled, convert to PwsDaoException
    throw new PwsDaoException(errorString, e);
  ...
  public static Object callbackHibernateSession(
      PersonWsTransactionType transactionType, PwsAuditControl auditControl, HibernateHandler hibernateHandler)
      throws PwsDaoException {
    Object ret = null;
    ...
    try {
      ...
      boolean willCreateAudit = PwsAuditControl.WILL_AUDIT.equals(auditControl);
   ...
   */
  public PersonWsTransactionType getTransactionType() {
