 ...
 * </pre>
 */
public class PwsKerberosAuthentication implements PwsCustomAuthentication {
...
//    }
  ...
  /** logger */
  private static final Log LOG = LogFactory.getLog(PwsKerberosAuthentication.class);
  ...
  private static ExpirableCache<String, Boolean> loginSuccessCache = new ExpirableCache<String, Boolean>(
    PersonWebServiceServerConfig.retrieveConfig().propertyValueInt("kerberos.login.cache.success.minutes", 5));

  /**
   * cache the logins in a hash cache
   */
  private static ExpirableCache<String, Boolean> loginFailedCache = new ExpirableCache<String, Boolean>(
      PersonWebServiceServerConfig.retrieveConfig().propertyValueInt("kerberos.login.cache.failed.minutes", 1));
  ...
  public static boolean authenticateKerberos(String principal, String password) {
    //hash the authHeader
    String authHeaderHash = PersonWsServerUtils.encryptSha(principal + USER_PASS_SEPARATOR + password);
  ...
  private static boolean authenticateKerberosHelper(String principal, String password) {
    ...
    try {
      lc = new LoginContext("JaasSample", new PersonWsKerberosHandler(principal, password));
   ...
   * cache the logins in a hash cache
   */
  private static PersonWsCache<String, String> loginCache = new PersonWsCache<String, String>(
      PwsKerberosAuthentication.class.getName() + ".userCache", 10000, false, 60*1, 60*1, false);
  ...
  public String retrieveLoggedInSubjectId(HttpServletRequest httpServletRequest)
    ...
    //hash the authHeader
    String authHeaderHash = PersonWsServerUtils.encryptSha(authHeader);
    ...
    String user = PersonWsServerUtils.prefixOrSuffix(unencodedString, ":", true);
    String pass = PersonWsServerUtils.prefixOrSuffix(unencodedString, ":", false);
