...
/*
 * @author mchyzer
 * $Id: XmlIndenter.java,v 1.1 2013/06/20 06:02:50 mchyzer Exp $
 */
...
public class XmlIndenter {
  ...
  public String result() {
    ...
    return PersonWsServerUtils.trim(this.result.toString());
  }
  ...
  private void newlineIndent() {
    this.result.append("\n").append(PersonWsServerUtils.repeat("  ", this.currentNumberOfIndents));
  }
  ...
  static boolean textTag(String xml, int endTagIndex, String tagName, 
    ...
    if (PersonWsServerUtils.equals("XML_COMMENT", tagName)) {
      return false;
    }
    if (PersonWsServerUtils.equals(tagName, nextTagName) && isNextCloseTag) {
      return true;
    }
    return false;
  }
  ...
  public XmlIndenter(String theXml) {
    if (theXml != null) {
      this.xml = PersonWsServerUtils.trimToEmpty(theXml);
    }
  }
