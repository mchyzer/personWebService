...
//    }
    ...
    for (int i=0;i<SECRET_FILE_SIZE;i++) {
      thingsToEncrypt[i] = PersonWsServerUtils.uuid() + PersonWsServerUtils.uuid() + PersonWsServerUtils.uuid() + PersonWsServerUtils.uuid();
    }
  ...
  private static String[] encryptionKeys() {
    ...
    synchronized (EncryptionKey.class) {
      ...
      //rebuild
      String encryptionFileLocation  = PersonWebServiceServerConfig.retrieveConfig()
        .propertyValueStringRequired("personWsServer.encryptKeyFileLocation");
      
      InputStream inputStream = PersonWsServerUtils.fileOrClasspathInputstream(encryptionFileLocation, 
      ...
      if (inputStream == null) {
        throw new RuntimeException("Cant find encryption keys.  You should encrypt with GenerateKeys.main(), save to: " 
            + PersonWebServiceServerConfig.retrieveConfig().propertyValueStringRequired("personWsServer.encryptKeyFileLocation"));
      }
      ...
      try {
        ...
        PersonWsServerUtils.closeQuietly(inputStream);
      }
   ...
   * @return the encrypted string
   */
  public static String encrypt(String data, PwsSymmetricEncryption tfSymmetricEncryption) {
   ...
   * @return the decrypted string
   */
  public static String decrypt(String timestampAndEncryptedData, PwsSymmetricEncryption tfSymmetricEncryption) {
   ...
   */
  public static PwsSymmetricEncryption symmetricEncryptionProvider() {
    String symmeticEncryptionProviderClassName = PersonWebServiceServerConfig.retrieveConfig().propertyValueString(
        "personWsServer.symmetricEncryptionProvider", PwsSymmetricEncryptAesCbcPkcs5Padding.class.getName());
    Class<PwsSymmetricEncryption> symmetricEncryptionProviderClass = PersonWsServerUtils.forName(symmeticEncryptionProviderClassName);
    return PersonWsServerUtils.newInstance(symmetricEncryptionProviderClass);
  }
